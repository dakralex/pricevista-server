use crate::providers::algolia::AlgoliaBrowseResponse;
use crate::providers::markant::MarkantAttributes;
...
const ALGOLIA_API_KEY: &str = "6d27574257fd3a92542ff880585333f1";
const ALGOLIA_APP_ID: &str = "UZXORS8TL2";
const ALGOLIA_INDEX: &str = "prod_mpreis_8450";

impl Fetch for MpreisShopApi {
    type ResponseImpl = MpreisBrowseResponse;
    const API_BASE_URL: &'static str = "https://uzxors8tl2-dsn.algolia.net/1";

    async fn fetch(client: &Client) -> crate::error::Result<Self::ResponseImpl> {
        let mut total = MpreisBrowseResponse::default();
        let url = format!("{}/indexes/{}/browse", Self::API_BASE_URL, ALGOLIA_INDEX);
        ...
        loop {
            ...
            if let Some(cursor) = &total.cursor {
                request = request.query(&[("cursor", cursor)]);
            }

            let response = request.send().await?;
            total.merge(response.json::<MpreisBrowseResponse>().await?);
